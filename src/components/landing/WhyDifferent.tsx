  ...
  return (
    <section className="py-24 px-6">
      <div ref={ref} className="scroll-reveal max-w-2xl mx-auto text-center">
        <TwinkleText as="h2" intensity="aura" className="text-3xl font-bold mb-3 block" style={{ fontFamily: 'Cinzel, serif', color: 'hsl(35 25% 88%)' }}>{t("pages:ui.whyDifferent.title", "Why This Guru Is Different")}</TwinkleText>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '1px solid hsl(38 78% 55% / 0.1)' }}>
                <th className="text-left p-4 font-normal" style={{ color: 'hsl(35 12% 55%)' }}>{t("pages:ui.whyDifferent.feature", "Feature")}</th>
                <th className="p-4 font-normal" style={{ color: 'hsl(35 12% 55%)' }}>{t("pages:ui.whyDifferent.mostApps", "Most Apps")}</th>
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr key={i} style={i < rows.length - 1 ? { borderBottom: '1px solid hsl(38 78% 55% / 0.08)' } : {}}>
                  <td className="text-left p-4" style={{ color: 'hsl(35 25% 88%)' }}>{t("pages:ui.whyDifferent." + row.key, row.label)}</td>
