...
function computeDashaTransitions(chartData: any, birthDate: string): TimelineEvent[] {
  ...
  for (let i = 0; i < 9; i++) {
    ...
    if (antarStart > now && antarStart <= oneYearLater) {
      ...
      events.push({
        ...
        title_key: "antarShift",
        title_params: { lord: antarLord },
        description: "",
        planet: antarLord,
  ...
  if (mahaEnd > now && mahaEnd <= oneYearLater) {
    ...
    events.push({
      ...
      title_key: "mahaChange",
      title_params: { from: mahaDashaLord, to: nextMahaLord },
      description: "",
      planet: nextMahaLord,
    ...
    for (let dayOffset = 0; dayOffset <= 370; dayOffset += 7) {
      ...
      if (prevSign && sign !== prevSign) {
        ...
        events.push({
          ...
          title_key: "ingress",
          title_params: { planet: pl.name, sign },
          description: "",
          planet: pl.name,
        });
      }
  ...
  for (let dayOffset = 0; dayOffset <= 370; dayOffset += 14) {
    ...
    if (prevRahuSign && rahuSign !== prevRahuSign) {
      ...
      events.push({
        ...
        title_key: "nodesIngress",
        title_params: { rahuSign, ketuSign },
        description: "",
        planet: "Rahu",
      });
    }
